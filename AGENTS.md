...
1. **Port/Adapter (Hexagonal):** O código de negócio e feature SÓ conhece portas/interfaces (`common/ports/`). A implementação real de bibliotecas (`axios`, `zustand`, `queries`) fica isolada em `infra/`.
...
│   ├── http/             # Ex: axios.adapter.ts implementando o http.port
